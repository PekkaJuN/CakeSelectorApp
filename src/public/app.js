...
// When product is selected, fetch and display properties
document.getElementById('product-select').addEventListener('change', async (e) => {
  ...
  try {
    const response = await fetch(`/api/products/${productId}/properties`);
    ...
    // Build property dropdowns
    container.innerHTML = properties.map(prop => `
      <div class="form-group">
        <label for="prop-${prop.id}">${escapeHtml(prop.name)}:</label>
        <select id="prop-${prop.id}" data-property-id="${prop.id}" data-property-name="${escapeHtml(prop.name)}">
          <option value="">-- Select ${escapeHtml(prop.name).toLowerCase()} --</option>
        </select>
      </div>
    `).join('');
    ...
    for (const prop of properties) {
      const propResponse = await fetch(`/api/properties/${prop.id}/values`);
      if (!propResponse.ok) continue;
      const values = await propResponse.json();

      const select = document.getElementById(`prop-${prop.id}`);
...
document.getElementById('add-item-btn').addEventListener('click', async () => {
  ...
  // Collect property selections
  const propertiesContainer = document.getElementById('properties-container');
  const selections = {};
  let allSelected = true;

  if (propertiesContainer.style.display !== 'none') {
    const selects = propertiesContainer.querySelectorAll('select');
    selects.forEach(select => {
      const propId = select.dataset.propertyId;
      const value = select.value;
      if (!value) {
        allSelected = false;
      } else {
        selections[propId] = value;
      }
    });
  }

  if (!allSelected) {
    errorDiv.textContent = 'All properties must be selected';
    return;
  }
    ...
    <div class="cart-item">
      <div class="item-details">
        ...
        <div class="item-properties">
          ${Object.entries(item.selections).map(([propId, valueId]) => {
            // Get property and value names from the dropdowns or stored data
            return `<span>${valueId}</span>`;
          }).join(', ')}
        </div>
      </div>
      <button onclick="removeCartItem('${item.id}')" class="btn-secondary">Remove</button>
    </div>
  `).join('');
}
